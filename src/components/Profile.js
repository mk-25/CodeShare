...
function Profile() {
  ...
  useEffect(()=>{
    ...
    const fetchUser = async()=>{
      ...
      try{
        const res = await axios.get("https://stormy-newt-getup.cyclic.cloud//api/private",config);
      ...
      const fetchPosts = async ()=>{
        const res = await axios.get(`https://stormy-newt-getup.cyclic.cloud//api/posts/mail/${user.email}`)
        setPosts(res.data);
      }
      fetchPosts();
  ...
  const handleDelete = async(id)=>{
    ...
    if(ans==="Y" || ans==="y"){
      try{
          await axios.delete(`https://stormy-newt-getup.cyclic.cloud//api/posts/${id}/${user?.email}`);
      }catch(err){
